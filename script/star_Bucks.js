(function($, window){
    ...
    var starBucks = {
          ...
          header:function(){
              ...
              // 반응형으로 작업해야함.
              // 화면 너비가 960 초과이면 left:100+'%'가 안으로 들어간다. 즉 메뉴가 사라진다.
              var t = false;
              $(window).resize(function(){
                if( $(window).width()>960 ){
                  mobileNavfn(); 
                  // 강제클릭 : trigger 제이쿼리의 강제 명령
                  // $('.mobile-close').trigger('이벤트');
                 /*  if( t==false ){ ❤ 1회만 수행하도록. 안그러면 크기가 커질 때 마다 무한정 실행
                    t=true;
              ...
              $('.mobile-container li a').on({
                click: function(){
                  ...
                  $('.mobile-container li a.none-sub').removeClass('addMobile'); // 하위항목 없는 메뉴에 화살표 애니메이션 없도록.
                }
              });
